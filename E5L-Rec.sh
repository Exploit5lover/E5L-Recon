bash
   ...
   install_tools() {
       ...
       tools=(subfinder assetfinder amass httpx hakrawler gau waybackurls gf github-subdomains github-endpoints nuclei)
   ...
   # Function to run vulnerability scanning with nuclei (optional)
   run_vulnerability_scan() {
       local urls_file=$1
       local output_dir=$2

       if command_exists "nuclei"; then
           echo -e "${GREEN}[+] Running vulnerability scan with nuclei...${NC}"
           cat "$urls_file" | nuclei -o "$output_dir/nuclei_results.txt" 2>/dev/null
       else
           echo -e "${RED}[-] nuclei is not installed. Skipping vulnerability scan.${NC}"
       fi
   }
   ...
   generate_report() {
       ...
       if [ -f "$output_dir/nuclei_results.txt" ]; then
           echo "Vulnerabilities found: $(wc -l < "$output_dir/nuclei_results.txt" 2>/dev/null || echo 0)" >> "$output_dir/report.txt"
       fi
       echo "" >> "$output_dir/report.txt"
       ...
       if [ -f "$output_dir/nuclei_results.txt" ]; then
           echo "- nuclei_results.txt: Vulnerability scan results" >> "$output_dir/report.txt"
       fi
   }
   ...
   if [ "${RUN_VULNERABILITY_SCAN}" = "true" ]; then
       run_vulnerability_scan "$OUTPUT_DIR/all_urls.txt" "$OUTPUT_DIR"
   fi
   ...
   if [ -f "$OUTPUT_DIR/nuclei_results.txt" ]; then
       echo "- nuclei_results.txt → Vulnerability scan results"
   fi
